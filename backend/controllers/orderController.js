...
exports.createOrder = async (req, res) => {
  try {
    ...
    const order = new Order({
      ...
      status: req.body.status || 'payment-pending'
    });
    ...
    // (the accepted one already has status 'accepted' — leave it untouched)
    ...
    // Mark the WINNING proposal as 'order-created' so the UI can show the
    // correct badge even after a page refresh (persisted to DB)
    await Proposal.findOneAndUpdate(
      { projectId: jobId, freelancerId: freelancerId, status: 'accepted' },
